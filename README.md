...
So far just testing with the dynamixels to see if we can actually get torque transparency. I think whatever method is used to guess dynamixel torque (not a load cell) causes it to be really noisy during movement. You can sorta see in the plots how applying a constant-ish torque results in a signal that keeps jumping down to zero. The filtering helps a little bit with preventing oscillation while standing still but free space movement feels not great.
